...
const Profile = React.createClass({
  ...
  getInitialState() {
    return {user: UserStore.getDefaultData(), profileData: ProfileStore.getDefaultData(), loading: true};
  },
  ...
  componentDidMount() {
    ...
    UserStore.getUserId(username).then(Actions.watchProfile);
  },
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (oldUsername !== newUsername) {
      ...
      UserStore.getUserId(newUsername).then(Actions.watchProfile);
    }
  },
  ...
  render() {
    ...
    if (loading) {
      ...
      postHeader = <h2>{posts.length
          ? 'Latest'
          : 'No'}
        Posts</h2>;
      commentsHeader = <h2>{comments.length
          ? 'Latest'
          : 'No'}
        Comments</h2>;

      postList = posts.map(post => (<Post post={post} user={user} key={post.id}/>));

      commentList = comments.map(comment => (<Comment showPostTitle comment={comment} user={user} key={comment.id}/>));
    }

    let userOptions = user.uid === profileData.userId && (
      <div className="user-options text-right">
        <button onClick={this.logout} className="button button-primary">
          Sign Out
        </button>
        <hr/>
      </div>
    ...
    return (
      <div className="content full-width">
        {userOptions}
        <h1>{this.props.params.username + '\'s'
        }
        Profile < /h1>
                <div className="user-posts">
                    { postHeader }
                    { postList }
                </div > <div className="user-comments">
          {commentsHeader}
          {commentList}
        </div> < /div}
    );
